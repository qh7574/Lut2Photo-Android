...
![应用版本](https://img.shields.io/badge/版本-3.3.0-blue)
...
![Gradle](https://img.shields.io/badge/Gradle-8.13-green)
![ZoomImage](https://img.shields.io/badge/ZoomImage-1.1.0-purple)
...
- 相机通过各种app或数据线配置自动传输至手机，app 监控对应文件夹实现自动叠加Lut处理
- 手动从相册选择需要处理的图片进行处理

**⚠️注意：app 未内置 raw 解码器，选择 raw 文件实际处理的是内嵌的低尺寸 jpg。**

2. **进阶使用**方法：

- 松下相机无 Lut 直出但支持 vlog 助手的机型（G9\S5\S1等），可将同一个 .vlt 文件同时导入相机内和
  Lut2Photo 中，同时实现监看和自动处理。
- 若你的 lut 文件基于其他色彩空间，可在 Lut1 中选择对应的转换 Lut，并在Lut2中设置所需的风格化即可获得准确输出，app
  内置 Vlog-709 和 srgb-709 转换 Lut，其他转换 Lut 可自行制作导入。
...
> 📖 **详细说明**：[胶片颗粒参数详解](胶片颗粒参数详解.md)


### 🔧 技术特性

- **GPU加速处理**：V3.3.0+ 已迁移至vulkan api进行硬件加速
...
|  **Vulkan**  | 强制使用GPU处理 | 高性能需求  |  最快，但部分设备不支持  |
...
- **ZoomImage** `1.1.0` - 图片缩放查看
  - 链接：https://github.com/panpf/zoomimage
  - 用途：预览图片全屏查看功能
