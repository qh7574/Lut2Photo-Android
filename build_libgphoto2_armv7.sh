#!/bin/bash
# libgphoto2 交叉编译脚本 for Android ARMv7
...
if [ ! -f "$CC" ]; then
    log_error "编译器不存在: $CC"
    log_info "请检查 NDK 路径和版本"
    exit 1
fi

# 关键：添加 __ANDROID__ 宏以启用 Android USB 支持
export CFLAGS="-fPIC -ffunction-sections -funwind-tables -fstack-protector-strong -no-canonical-prefixes -DANDROID -D__ANDROID__ -DNDEBUG -O2 -march=armv7-a -mfloat-abi=softfp -mfpu=neon"
export CPPFLAGS="-I${TOOLCHAIN}/sysroot/usr/include -I${TOOLCHAIN}/sysroot/usr/include/arm-linux-androideabi -D__ANDROID__"
export LDFLAGS="-L${TOOLCHAIN}/sysroot/usr/lib/arm-linux-androideabi/${API_LEVEL} -L${PREFIX}/lib -llog -march=armv7-a"
...
build_libusb() {
    ...
    # 使用 libusb 主分支，它包含完整的 Android 支持
    # 包括 libusb_wrap_sys_device() 和 LIBUSB_OPTION_NO_DEVICE_DISCOVERY
